...
One pracical benefit of this algorithm is that it may allow certain types of fitness functions to cache and build on previous results increasing real-world performance. This technique is used in some of the following examples to produce stuctures with n>3000 in under 15 seconds on modest hardware using a fairly slow interpretted language. This could likely be done in realtime if properly optimized.
...
Here is are a few example rules to construct a wall with the irregular appearance of a sheer rock face:
